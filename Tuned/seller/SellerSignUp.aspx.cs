    ...
    public partial class SellerSignUp : System.Web.UI.Page
    {
        ...
        protected void Register_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("insert into[SellerDetails] (email, cno, password) values('" + email + "', '" + contact + "', '" + password + "')", con);	    	//query statement preparing to be sent to the database
                cmd.ExecuteNonQuery();		                        //inserting data into database
                con.Close();		                                //closing the database connection
                Response.Redirect("~/SellerLogin.aspx");		            //Redirects the user to the Home page
            }
