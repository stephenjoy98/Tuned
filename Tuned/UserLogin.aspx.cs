...
namespace Tuned.user
{
    public partial class UserLogin : System.Web.UI.Page
    {
        ...
        protected void Login_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("select * from [UserDetails] where email = '" + email + "'and password = '" + password + "'", con);		//query statement that has to be sent to the database
            ...
            if (ds.Tables[0].Rows.Count > 0)		                    //traversing the rows and cross checking the input
            {
                Session["login"] = email;
                Session["role"] = role;
                Response.Redirect("~/user/UserProfile.aspx");		                //Redirects the user to the Home page
            }
            else
            {
