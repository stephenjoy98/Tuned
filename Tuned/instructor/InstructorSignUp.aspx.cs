    ...
    public partial class InstructorSignUp : System.Web.UI.Page
    {
        ...
        protected void Register_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string password = Hash.CreateMD5Hash(TextBox4.Text);     //calling the MD5 function to encrypt password
                ...
                SqlCommand cmd = new SqlCommand("insert into[InstructorDetails] (email, cno, password) values('" + email + "', '" + contact + "', '" + password + "')", con);	    	//query statement preparing to be sent to the database
                cmd.ExecuteNonQuery();		                        //inserting data into database
                con.Close();		                                //closing the database connection
                Response.Redirect("~/InstructorLogin.aspx");		            //Redirects the user to the Home page
            }
